...
export const getMultipleAuctionStates = async (
  auctionAddrs: Address[],
  publicClient: PublicClient,
): Promise<AuctionState[]> => {
  if (auctionAddrs.length === 0) return [];
  ...
  const states = await lens.read.getMultipleAuctionStates([auctionAddrs]);
